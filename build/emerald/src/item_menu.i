...
static const struct MenuAction sItemMenuActions[] = {
    ...
    [8] = {gMenuText_Deselect, ItemMenu_Register},
...
struct ListBuffer1 {
    struct ListMenuItem subBuffers[(64 + 1)];
};
...
void GetItemName(s8 *dest, u16 itemId)
{
    switch (gBagPositionStruct.pocket)
    {
        case 2:
            ...
            if (itemId >= 532)
            {
                ConvertIntToDecimalStringN(gStringVar1, itemId - 532 + 1, STR_CONV_MODE_LEADING_ZEROS, 1);
                StringExpandPlaceholders(dest, gText_ClearTo11Var1Clear5Var2);
            }
            else
            {
                ConvertIntToDecimalStringN(gStringVar1, itemId - 482 + 1, STR_CONV_MODE_LEADING_ZEROS, 2);
...
void BagMenu_ItemPrintCallback(u8 windowId, s32 itemIndex, u8 y)
{
    ...
    if (itemIndex != -2)
    {
        ...
        else if (gBagPositionStruct.pocket != 4 && ItemId_GetImportance(itemId) == 0)
        {
