...
static u8 *GetConditionMenuMonString(u8 *dst, u16 boxId, u16 monId)
{
    ...
    *(dst++) = 0x0;
    *(dst++) = 0x9;
    if (GetBoxOrPartyMonData(box, mon, 45, ((void *)0)))
        return StringCopyPadded(dst, gText_EggNickname, 0, 12);
    GetBoxOrPartyMonData(box, mon, 2, dst);
    StringGetEnd10(dst);
    species = GetBoxOrPartyMonData(box, mon, 11, ((void *)0));
    if (box == 14)
    {
        level = GetMonData(&gPlayerParty[mon], 56);
