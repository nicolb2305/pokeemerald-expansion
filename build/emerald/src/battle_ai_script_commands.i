...
# 33 "src/battle_ai_script_commands.c"
enum
{
    AIState_SettingUp,
    AIState_Processing,
...
# 49 "src/battle_ai_script_commands.c"
...
void BattleAI_SetupItems(void)
{
    ...
    if ((gBattleTypeFlags & (1 << 3))
        && !(gBattleTypeFlags & ((1 << 1) | (1 << 7) | (1 << 8)
                               | (1 << 11) | (1 << 27) | ((1 << 8) | (1 << 16) | (1 << 17) | (1 << 18) | (1 << 19) | (1 << 20) | (1 << 21))
                               | (1 << 22) | (1 << 25))
            )
       )
    {
...
void BattleAI_SetupFlags(void)
{
    ...
    if (gBattleTypeFlags & ((1 << 0) | (1 << 15)) || gTrainers[gTrainerBattleOpponent_A].doubleBattle)
...
void ClearBattlerMoveHistory(u8 battlerId)
{
    memset(((struct BattleHistory *)(gBattleResources->battleHistory))->usedMoves[battlerId], 0, sizeof(((struct BattleHistory *)(gBattleResources->battleHistory))->usedMoves[battlerId]));
    memset(((struct BattleHistory *)(gBattleResources->battleHistory))->moveHistory[battlerId], 0, sizeof(((struct BattleHistory *)(gBattleResources->battleHistory))->moveHistory[battlerId]));
    ((struct BattleHistory *)(gBattleResources->battleHistory))->moveHistoryIndex[battlerId] = 0;
}

void RecordLastUsedMoveBy(u32 battlerId, u32 move)
{
    u8 *index = &((struct BattleHistory *)(gBattleResources->battleHistory))->moveHistoryIndex[battlerId];

    if (++(*index) >= 3)
        *index = 0;
    ((struct BattleHistory *)(gBattleResources->battleHistory))->moveHistory[battlerId][*index] = move;
}
...
void RecordKnownMove(u8 battlerId, u32 move)
{
    ...
    for (i = 0; i < 4; i++)
    {
        if (((struct BattleHistory *)(gBattleResources->battleHistory))->usedMoves[battlerId][i] == move)
            break;
        if (((struct BattleHistory *)(gBattleResources->battleHistory))->usedMoves[battlerId][i] == 0)
        {
            ((struct BattleHistory *)(gBattleResources->battleHistory))->usedMoves[battlerId][i] = move;
            break;
        }
    }
}
...
static void Cmd_if_has_move(void)
{
    ...
    case 2:
        for (i = 0; i < 4; i++)
        {
            if (((struct BattleHistory *)(gBattleResources->battleHistory))->usedMoves[gBattlerTarget][i] == *movePtr)
                break;
        }
        if (i == 4)
            gAIScriptPtr += 8;
...
static void Cmd_if_has_move_with_effect(void)
{
    ...
    case 2:
        for (i = 0; i < 4; i++)
        {
            if (gBattleMons[gBattlerTarget].moves[i] != 0 && gBattleMoves[((struct BattleHistory *)(gBattleResources->battleHistory))->usedMoves[gBattlerTarget][i]].effect == gAIScriptPtr[2])
                break;
        }
        if (i == 4)
            gAIScriptPtr += 7;
...
static void Cmd_if_doesnt_have_move_with_effect(void)
{
    ...
    case 2:
        for (i = 0; i < 4; i++)
        {
            if (((struct BattleHistory *)(gBattleResources->battleHistory))->usedMoves[gBattlerTarget][i] && gBattleMoves[((struct BattleHistory *)(gBattleResources->battleHistory))->usedMoves[gBattlerTarget][i]].effect == gAIScriptPtr[2])
                break;
        }
        if (i != 4)
            gAIScriptPtr += 7;
...
static void Cmd_if_has_no_attacking_moves(void)
{
    ...
    else
    {
        for (i = 0; i < 4; i++)
        {
            if (((struct BattleHistory *)(gBattleResources->battleHistory))->usedMoves[battlerId][i] != 0 && gBattleMoves[((struct BattleHistory *)(gBattleResources->battleHistory))->usedMoves[battlerId][i]].power != 0)
                break;
        }
    }
...
static void Cmd_if_no_move_used(void)
{
    ...
    if (!IsBattlerAIControlled(battler))
    {
        for (i = 0; i < 4; i++)
        {
            if (((struct BattleHistory *)(gBattleResources->battleHistory))->usedMoves[battler][i] != 0 && ((struct BattleHistory *)(gBattleResources->battleHistory))->usedMoves[battler][i] != 0xFFFF)
            {
                gAIScriptPtr += 6;
                return;
            }
...
{
    s32 i, index = ((struct BattleHistory *)(gBattleResources->battleHistory))->moveHistoryIndex[battlerId];
    for (i = 0; i < x; i++)
    {
        if (--index < 0)
            index = 3 - 1;
    }
    return ((struct BattleHistory *)(gBattleResources->battleHistory))->moveHistory[battlerId][index];
}
